...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($existing) {
        ...
        $sql = "INSERT INTO parent (name, email, password, phone, address) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        if ($stmt->execute([$name, $email, $password, $phone, $address])) {
            $success = "Parent registered successfully! You can now <a href='../auth/parent_login.php'>login</a>.";
        } else {
...
<title>Parent Registration - VaxPoint</title>
<style>
  body, html {
    margin: 0; padding: 0;
    height: 100%;
  ...
  .register-form {
    ...
    padding: 30px 35px;
    border-radius: 16px;
    box-shadow: 0 12px 30px rgba(0,0,0,0.5); /* Stronger shadow to pop out */
    width: 400px;
    max-width: 90vw;
  }
...
<div class="register-form">
  ...
  <p class="login-link">
    Already have an account? <a href="../auth/parent_login.php">Login here</a>
  </p>
</div>
